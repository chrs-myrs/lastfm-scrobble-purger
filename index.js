...
const request = limit(require("request")).to(50).per(1000);
...
const csfrtoken = config.csfrtoken
const sessionid = config.sessionid
const apiKey = config.apiKey
const artistsToRemove = config.artistsToRemove

var totalQueued = 0;
var totalDeleted = 0;

for (var page = startPage; page <= endPage; page++) {
    console.log("Requesting page " + page + " of " + (endPage - startPage + 1))
    var options = {
        method: 'GET',
        ...
        headers: { 'Postman-Token': 'fac4d239-9b0e-42a7-9b2e-cc25ccdc7ac2' }
    };
    ...
    (function (page) {
        request(options, function (error, response, body) {
            ...
            var resTracks = JSON.parse(body).recenttracks.track
            console.log("Page " + page + ": " + resTracks.length + " tracks in response.")
            resTracks.filter(
                t => (artistsToRemove.indexOf(t.artist['#text']) >= 0)
            ).map(
                t => {
                    return {
                ...
                .forEach(function (unscrobbleTrack, index, all) {
                    console.log("Deleting scrobble " + (index + 1) + " of " + all.length + ": " + unscrobbleTrack.track_name + " @ " + (new Date(unscrobbleTrack.timestamp * 1000)).toUTCString())
                    if (simulationMode) {
                        console.log("Simulation: " + JSON.stringify(unscrobbleTrack));
                    } else {
                        unscrobble(unscrobbleTrack, page)
                    }
                    totalQueued++;
                });;
...
function unscrobble(trackData, page) {
    ...
    (function (trackData, page) {
        try {
            request(options, function (error, response, body) {
                if (error) console.log("Page " + page + ": Error deleting " + JSON.stringify(trackData) + " - " + error)
                if (!JSON.parse(body).result) {
                    console.log("Page " + page + ": Error deleting " + JSON.stringify(trackData))
                } else {
                    totalDeleted++;
                }
            ...
            console.log("Page " + page + ": Error deleting " + JSON.stringify(trackData) + " - " + error)
        }
